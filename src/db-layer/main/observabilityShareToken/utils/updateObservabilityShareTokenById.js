...
const indexDataToElastic = async (data) => {
  const elasticIndexer = new ElasticIndexer("observabilityShareToken");
  await elasticIndexer.indexData(data);
};

const updateObservabilityShareTokenById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await ObservabilityShareToken.findOne({
    ...
    const [rowsCount, [dbDoc]] = await ObservabilityShareToken.update(
      dataClause,
      options,
    );
    if (!dbDoc) {
...
module.exports = updateObservabilityShareTokenById;
